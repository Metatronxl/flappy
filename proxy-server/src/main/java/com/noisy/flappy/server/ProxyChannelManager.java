...
public class ProxyChannelManager {
        ...
        ProxyConfig.getInstance().addConfigChangedListener(new ProxyConfig.ConfigChangedListener() {
            ...
            public void onChanged() {
                ...
                while (ite.hasNext()) {
                    ...
                    log.info("proxyChannel config, {}, {}, {} ,{}", entry.getKey(), proxyChannel, getUserChannels(proxyChannel).size(), proxyChannel.attr(CHANNEL_PORT).get());
                }
            }
     ...
     * (一个代理控制客户端的不同服务会占用代理服务器的不同端口)
     * (代理控制客户端通过cmdChannel 与代理服务器进行通信)
     * @param ports
     * @param channel
     */
